...
```
Ещё с помощью auto удобно ходить по карте через for(:). Ну, чтобы не писать for(pair<string, unordered_set<double>> item : map_of_coefficents). Важно понимать, что C++ - это язык со статической типизацией, то есть все типы в коде определяются в момент компиляции, и при запуске программы уже известны и не меняются. То есть тип, скрывающийся под auto, должен быть виден по тексту кода.
В целом прелести auto заканчиваются, но нам большего от него и не надо.
* Современное: decltype с примером.
decltype - определение типа переданного выражения в compile-time.
	
```bash
 class MyAwesomeDataTypeValue
  {
protected:
    std::shared_ptr<std::pair<std::unordered_map<unsigned long long, long double>::reverse_iterator *, std::string::const_iterator **> ***> value{}
...
```
* Современное: range base loop с примером.
```bash
std::string myVeryLongGoldString{"I am a string!"};

for (const char *c : myVeryLongGoldString) {    // сюда можно вставить auto
  std::cout << "Char: " << c << "\n";
}
```
* Современное: nullptr с примером.
> nullptr - это нулевой указатель, то есть указатель, который никуда не указывает. Он по-честному равен нулю, но он не может быть неявно преобразован к целочисленным типам и вообще позволяет программисту сходу опознать переменную как указатель, а не как bool или int.

Пример использования:
```bash
  vector<void(*)()> jobs;     // Создали вектор функций, которые нам надо выполнить (смотри вопрос 12)
  vector.resize(n, nullptr);  // Лоховской пример, потому что вектор и так инициализирует создаваемые места нулём
  //...                       // закладывание каких-то работ в вектор
  for(auto thing : jobs){
      if(thing==nullptr) continue;   // а если бы оставили валяться мусор, то программа бы попыталась его выполнить и упала
      thing();                       // а так делаем только всё, что реально существует
  }
```
* Современное: constexpr с примером.
> constexpr - это ключевое слово, которое можно написать перед типом возвращаемого значения функции или типом переменной.

Тогда мы получим constexpr-функцию или constexpr-переменную.
	Функция: вычисляет себя на этапе компиляции для всех случаев, когда так можно (когда в неё передаются константы или constexpr-		переменные).
	Переменная: должна быть тут же инициализирована constexpr-функцией или у неё вызван constexpr-конструктор
Как правило используется для подбора подходящей перегрузки шаблонов (SFINAE)
Пример1:
```bash
template <typename T>
constexpr bool isNotCopyConstructible()
{
...
Пример2:
```bash
constexpr size_t _getIntSize() {
	return sizeof(int);    // размер int на изях ищется на этапе компиляции
}


template<int a>
union intGetter {    // шаблонный класс для побайтового взлома int-ов
	int i;
	byte b[a];       // массив постоянного размера, я попрошу заметить
	// byte - это такой аналог char из С++17. Не ругайтесь, что "сложна-нипанятна", я всё пояснил только что
};


int main() {
	intGetter<_getIntSize()> r;    
	// если подставить в шаблон переменную, то он сломается,
	// но мы подставляем constexpr-функцию, которая считается на этапе компиляции, поэтому всё ок
}
```
* Современное: enum class с примером.
Все мы постоянно работаем с enum (перечисляемый тип) и всем нам не нравится, когда наша программа автоматически преобразует перечислители в целые числа. Постановка проблемы в коде
```bash
enum Seasons{     // программа умеет преобразовывать перечислители в int-ы (хотим мы этого или нет)
    winter,       // 0
    spring,       // 1
    summer,       // 2
    fall          // 3
...
int main(){
    if ( winter == MickeyMouse ) {           // это выражение истинно
		cout << "okay, thats weird";
	}
}
```
Ключевое слово class после enum
	*запрещает такие закулисные преобразования, и теперь перечислитель - это перечислитель, никаких внезапных int-ов. Если вам 		теперь вдруг такое понадобится, то каждое преобразование к типу int производится вручную через static_cast<int>()
	*запрещает использовать имена перечислителей (в примере: winter, spring..., MickeyMouse, DonaldDuck...) сами по себе, теперь к 		ним надо обращаться через имя типа: Seasons::winter и т.п. Это предотвращает конфликты имён
	*НЕ СВЯЗАНО С ОБЫЧНЫМИ КЛАССАМИ - никаких наследований, методов, конструкторов-деструкторов, полей и прочей аттрибутики
* Современное: if constexpr с примером.
Современное SFINAE
Позволяет производить ветвление кода на этапе компляции (в первую очередь используется шаблонами). Раньше писалось две шаблонные функции с разными специализациями шаблонов (специализация = вопрос 7). 
Теперь:
```bash
template<typename T>
std::string toString(T value)
{
    if constexpr (std::is_same<decltype(value), std::string>::value) {   // на этапе компиляции что-то делаем в зависимости от типа или свойств
        // здесь могут вызываться функции/методы, которые есть только у тех, кто заходит сюда по критерию
        return value;
    } else {
        // здесь могут вызываться функции/методы, которых нет для типов, которые не заходят в else
...
### Вопрос №10 ()
### Вопрос №11 ()
### Вопрос №12 (Лямбда-функции, функторы, указатели на функции, std::functional. Примеры использования std::functional, Примеры использования лямбда-функций)
